...
class UsersRouter extends _ClassesRouter2.default {
  ...
  handleMe(req) {
    if (!req.info || !req.info.sessionToken) {
      if(req.info){
        throw new _node2.default.Error(Parse.Error.INVALID_SESSION_TOKEN, 'invalid session token - SESSION TOKEN: ' + req.info.sessionToken);
      } else {
        throw new _node2.default.Error(Parse.Error.INVALID_SESSION_TOKEN, 'invalid session token - no req info');
      }
    }
