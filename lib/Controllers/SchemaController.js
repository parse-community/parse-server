...
exports.convertSchemaToAdapterSchema = exports.defaultColumns = exports.systemClasses = exports.buildMergedSchemaObject = exports.invalidClassNameMessage = exports.fieldNameIsValid = exports.classNameIsValid = exports.load = undefined;
...
var volatileClasses = Object.freeze(['_PushStatus', '_Hooks', '_GlobalConfig']);
...
var injectDefaultSchema = function injectDefaultSchema(_ref3) {
  ...
  var classLevelPermissions = _ref3.classLevelPermissions;
  return {
    className: className,
...
var SchemaController = function () {
  function SchemaController(databaseAdapter) {
  ...
  _createClass(SchemaController, [{
    ...
    value: function reloadData() {
      ...
      this.data = {};
      this.perms = {};
      return this.getAllClasses().then(function (allSchemas) {
    ...
    value: function getAllClasses() {
      return this._dbAdapter.getAllClasses().then(function (allSchemas) {
    ...
    value: function getOneSchema(className) {
      ...
      return this._dbAdapter.getClass(className).then(injectDefaultSchema);
    }
    ...
    value: function addClassIfNotExists(className) {
      ...
      return this._dbAdapter.createClass(className, convertSchemaToAdapterSchema({ fields: fields, classLevelPermissions: classLevelPermissions, className: className })).then(convertAdapterSchemaToParseSchema).catch(function (error) {
    ...
    value: function updateClass(className, submittedFields, classLevelPermissions, database) {
      ...
      return this.getOneSchema(className).then(function (schema) {
        ...
        var validationError = _this2.validateSchemaData(className, newSchema, classLevelPermissions, Object.keys(existingFields));
        if (validationError) {
        ...
        .then(function () {
          var promises = insertedFields.map(function (fieldName) {
            ...
            return _this2.enforceFieldExists(className, fieldName, type);
          });
    ...
    value: function enforceFieldExists(className, fieldName, type, freeze) {
      ...
      return this.reloadData().then(function () {
        var expectedType = _this5.getExpectedType(className, fieldName);
        if (typeof type === 'string') {
          type = { type: type };
        }
        ...
        if (expectedType) {
          if (!dbTypeMatchesObjectType(expectedType, type)) {
            throw new Parse.Error(Parse.Error.INCORRECT_TYPE, 'schema mismatch for ' + className + '.' + fieldName + '; expected ' + (expectedType.type || expectedType) + ' but got ' + type.type);
          }
        }

        return _this5._dbAdapter.addFieldIfNotExists(className, fieldName, type).then(function () {
          ...
          // Ensure that the schema now validates
          if (!dbTypeMatchesObjectType(_this5.getExpectedType(className, fieldName), type)) {
    ...
    value: function deleteField(fieldName, className, database) {
      ...
      return this.getOneSchema(className).catch(function (error) {
        if (error === undefined) {
          throw new Parse.Error(Parse.Error.INVALID_CLASS_NAME, 'Class ' + className + ' does not exist.');
        } else {
          throw error;
...
var load = function load(dbAdapter) {
  var schema = new SchemaController(dbAdapter);
