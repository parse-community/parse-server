...
var get = function get(config, auth, className, objectId, restOptions) {
  enforceRoleSecurity('get', className, auth);
  var query = new RestQuery(config, auth, className, { objectId: objectId }, restOptions);
  return query.execute();
};
...
function del(config, auth, className, objectId) {
  ...
  return Promise.resolve().then(function () {
    if (triggers.getTrigger(className, triggers.Types.beforeDelete, config.applicationId) || triggers.getTrigger(className, triggers.Types.afterDelete, config.applicationId) || config.liveQueryController && config.liveQueryController.hasLiveQuery(className) || className == '_Session') {
...
function update(config, auth, className, objectId, restObject) {
  ...
  return Promise.resolve().then(function () {
    if (triggers.getTrigger(className, triggers.Types.beforeSave, config.applicationId) || triggers.getTrigger(className, triggers.Types.afterSave, config.applicationId) || config.liveQueryController && config.liveQueryController.hasLiveQuery(className)) {
