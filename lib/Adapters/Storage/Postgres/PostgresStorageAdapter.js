...
var PostgresStorageAdapter = exports.PostgresStorageAdapter = function () {
  ...
  function PostgresStorageAdapter(_ref2) {
    ...
    var databaseOptions = _ref2.databaseOptions;
  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function _ensureSchemaCollectionExists() {
      return this._client.none('CREATE TABLE "_SCHEMA" ( "className" varChar(120), "schema" jsonb, "isParseClass" bool, PRIMARY KEY ("className") )').catch(function (error) {
    ...
    value: function createClass(className, schema) {
      ...
      return this.createTable(className, schema).then(function () {
        return _this2._client.none('INSERT INTO "_SCHEMA" ("className", "schema", "isParseClass") VALUES ($<className>, $<schema>, true)', { className: className, schema: schema });
      }).then(function () {
    ...
    value: function createTable(className, schema) {
      ...
      var qs = 'CREATE TABLE $1:name (' + patternsArray.join(',') + ')';
      var values = [className].concat(valuesArray);
      return this._ensureSchemaCollectionExists().then(function () {
        return _this3._client.none(qs, values);
        ...
        return Promise.all(relations.map(function (fieldName) {
          return _this3._client.none('CREATE TABLE IF NOT EXISTS $<joinTable:name> ("relatedId" varChar(120), "owningId" varChar(120), PRIMARY KEY("relatedId", "owningId") )', { joinTable: '_Join:' + fieldName + ':' + className });
        }));
      });
    }
  }, {
        ...
        if (type.type !== 'Relation') {
          promise = t.none('ALTER TABLE $<className:name> ADD COLUMN $<fieldName:name> $<postgresType:raw>', {
            ...
            if (error.code === PostgresRelationDoesNotExistError) {
              return _this4.createClass(className, { fields: _defineProperty({}, fieldName, type) });
    ...
    value: function deleteFields(className, schema, fieldNames) {
      ...
      return Promise.resolve().then(function () {
        ...
        return _this7._client.tx(function (t) {
    ...
    value: function getAllClasses() {
      ...
      return this._ensureSchemaCollectionExists().then(function () {
        return _this8._client.map('SELECT * FROM "_SCHEMA"', null, function (row) {
    ...
    value: function upsertOneObject(className, schema, query, update) {
      ...
      return this.createObject(className, schema, createValue).catch(function (err) {
        ...
        if (err.code === Parse.Error.DUPLICATE_VALUE) {
          return _this9.findOneAndUpdate(className, schema, query, update);
        }
        throw err;
      });
    }
    ...
    value: function performInitialization(_ref4) {
      ...
      var promises = VolatileClassesSchemas.map(function (schema) {
        return _this10.createTable(schema.className, schema).catch(function (err) {
