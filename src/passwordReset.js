      ...
      .then(()=> {
        ...
        return database.find('_User', {username: username})
          .then((results) => {
            ...
            if (user._perishable_token !== token) {
              return Promise.resolve("Invalid token: this is an invalid url")
            } else {
              return passwordCrypto.hash(password)
                .then((hashedPassword)=> {
                  return database.update("_User", {email: username}, {_hashed_password: hashedPassword, _perishable_token: cryptoUtils.randomString(25)}, {acl: [user.objectId]})
                })
                .then(()=> {
