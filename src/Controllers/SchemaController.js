  ...
  for (const operationKey in perms) {
    ...
    for (const entity in operation) {
      ...
      if (operationKey === 'ACL') {
        ...
        const invalidKeys = Object.keys(permit).filter(
          key => !['read', 'write'].includes(key)
        );
        const invalidValues = Object.values(permit).filter(
...
function fieldNameIsValid(fieldName: string, className: string): boolean {
  ...
  return (
    classAndFieldRegex.test(fieldName) && !invalidColumns.includes(fieldName)
  );
}
      ...
      Object.defineProperty(this, schema.className, {
        get: () => {
          if (!this.__data[schema.className]) {
            ...
            const classProtectedFields =
              this.__protectedFields[schema.className];
...
export default class SchemaController {
  ...
  enforceFieldExists(
    ...
    if (fieldName.indexOf('.') > 0) {
      ...
      if (
        isArrayIndex &&
        !['sentPerUTCOffset', 'failedPerUTCOffset'].includes(fieldName)
      ) {
        type = 'Array';
      } else {
        type = 'Object';
    ...
    if (expectedType) {
      ...
      if (
        isValidation ||
        JSON.stringify(expectedType) === JSON.stringify(type)
      ) {
        return undefined;
      }
