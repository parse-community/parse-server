...
- CI runs MongoDB on **replset** topology (7.0.16 and 8.0.4); most local setups are standalone. Transactions and change streams behave differently, so run `MONGODB_TOPOLOGY=replset npm test` for anything touching them.
