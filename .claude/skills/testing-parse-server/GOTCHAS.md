...
Procedure lives in [VERIFICATION.md](VERIFICATION.md), attack patterns in [ADVERSARIAL-QA.md](ADVERSARIAL-QA.md), verified commands/env/harness facts in [REFERENCE.md](REFERENCE.md). This file is only for traps already paid for - if a lesson generalises into procedure, move it there instead of duplicating it here.
...
- A spec that only passes because it ran after another spec is a CI failure waiting to happen: the suite is random-order and `afterEach` wipes all data, clears the schema cache, removes Cloud hooks and logs the user out. Create fixtures inside the test.
- `reconfigureServer({...})` with a non-empty config marks the harness dirty and it reboots the default server afterwards. Calling it in `beforeAll` instead of the test leaks config into whatever runs next.
- CI's two version-matrix jobs run MongoDB on **replset** topology (7.0.16 and 8.0.4); the Node-20/Node-22/Redis jobs run standalone. Most local setups are standalone-only. Transactions and change streams behave differently, so run `MONGODB_TOPOLOGY=replset npm test` for anything touching them.
- `PARSE_SERVER_LOG_LEVEL=debug` does NOT give you debug-level logs inside specs: `spec/helper.js` passes `verbose: !silent`, setting the var un-silences the server so `verbose` is true, and `configureLogger` applies `if (verbose) logLevel='verbose'` LAST, clobbering the level you asked for. It still works for the Postgres SQL tracer, which reads `process.env.PARSE_SERVER_LOG_LEVEL` directly in `PostgresClient.js`. Don't chase "why is my debug log missing".
- `types/*.d.ts` are generated by `npm run build:types` and hand edits get overwritten. The exception is `types/Options/index.d.ts`, which is maintained manually alongside `src/Options/index.js`; changing options also needs `npm run definitions` or `ci:definitionsCheck` fails.
