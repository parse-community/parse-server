  ...
  it('succeed in verifying password when username and email are provided and password matches hash with json payload REST API', done => {
    ...
    user
      ...
      .then(response => {
        ...
        expect(Object.hasOwnProperty.call(res, 'sessionToken')).toEqual(false);
        expect(Object.hasOwnProperty.call(res, 'password')).toEqual(false);
        done();
      })
      .catch(err => {
        fail(err);
  ...
  it('succeed to verify password when username and password provided in query string REST API', done => {
    ...
    user
      ...
      .then(response => {
        ...
        expect(Object.hasOwnProperty.call(body, 'sessionToken')).toEqual(false);
        expect(Object.hasOwnProperty.call(body, 'password')).toEqual(false);
        done();
      });
  });
