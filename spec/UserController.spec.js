    ...
    describe('parseFrameURL not provided', () => {
      it_id('61338330-eca7-4c33-8816-7ff05966f43b')(it)('uses publicServerURL', async () => {
        ...
        expect(emailOptions.link).toEqual(
          `http://www.example.com/apps/test/verify_email?token=${rawToken}`
        );
      });
    });
    ...
    describe('parseFrameURL provided', () => {
      it_id('673c2bb1-049e-4dda-b6be-88c866260036')(it)(
        ...
        async () => {
          ...
          expect(emailOptions.link).toEqual(
            `http://someother.example.com/handle-parse-iframe?link=%2Fapps%2Ftest%2Fverify_email&token=${rawToken}`
          );
        }
      );
    });
