...
describe('allowExpiredAuthDataToken option', () => {
  it('should accept true value', async () => {
    ...
    expect(Config.get(Parse.applicationId).allowExpiredAuthDataToken).toBe(
      true
    );
  });
  ...
  it('should enforce boolean values', async () => {
    ...
    for (const option of options) {
      await expectAsync(
        reconfigureServer({ allowExpiredAuthDataToken: option })
      ).toBeRejected();
    }
  });
});
...
describe('Parse.User testing', () => {
  ...
  describe('case insensitive signup not allowed', () => {
    ...
    describe('anonymous users', () => {
      it('should not fail on case insensitive matches', async () => {
        ...
        const logIn = id =>
          Parse.User.logInWith('anonymous', { authData: { id } });
  ...
  it('should not allow updates to hidden fields', async () => {
    ...
    await expectAsync(user.save()).toBeRejectedWith(
      new Parse.Error(
        ...
        'Invalid field name: _email_verify_token.'
      )
    );
  });
  ...
  it('should throw when enforcePrivateUsers is invalid', async () => {
    ...
    for (const option of options) {
      await expectAsync(
        reconfigureServer({ enforcePrivateUsers: option })
      ).toBeRejected();
    }
  });
...
describe('allowClientClassCreation option', () => {
  it('should enforce boolean values', async () => {
    ...
    for (const option of options) {
      await expectAsync(
        reconfigureServer({ allowClientClassCreation: option })
      ).toBeRejected();
    }
  });
  ...
  it('should accept false value', async () => {
    ...
    expect(Config.get(Parse.applicationId).allowClientClassCreation).toBe(
      false
    );
  });
