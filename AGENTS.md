...
- Work test-first: write the failing spec before the implementation, then make it pass. Either way the proof obligation is the same, a new spec has to fail without your `src/` change and pass with it. Revert `src/`, rebuild, and watch it fail before you trust it.
...
- Wrap new tests in `it_id`: `it_id('<uuid>')(it)('description', async () => { ... })`, with a freshly generated UUID (`uuidgen`). The ID lets deployments exclude individual tests via an exclusion list, and reviewers ask for it on new tests.
- Place a new test in the `describe` block that matches its subject, or add a new block. A test appended to whichever block happened to be nearby will be flagged in review.
- Never wait with a hardcoded timeout in a spec; it is flaky by construction. Poll for the condition instead.
...
- **Reuse the canonical value or helper instead of re-deriving it.** Before cloning data, building a request object or hard-coding a list, check whether the codebase already has the single source of truth for it (for example `getRequestObject` in `src/triggers.js` for trigger request objects). Duplicated derivations drift apart and are rejected in review, and so is the sibling mistake of applying a behaviour (a clone, a validation) in one code path but not its parallel path.
- **No global singletons.** Parse Server serves multiple apps from one process and is designed around dependency injection: per-app state lives on the config object and flows to request handlers from there. Module-level mutable state is rejected in review.
- **Prefer the codebase's existing idiom over inventing a mechanism.** If the rest of the code solves the problem one way, a new helper that solves it a novel way is a review objection even when it works.
- **Group code into logical blocks, with a short comment before each block explaining its purpose.** That is the commenting convention here: block-level "why" comments, not line-by-line narration and not uncommented walls of code.
- **Error messages are full sentences.** Capitalized, ending with a period, no abbreviations a user would not know.
- **Coverage must not drop, and must not be gamed.** New code needs tests that actually reach it. Never add `istanbul ignore` comments to raise coverage: dead code is made reachable by a test or removed, not hidden from the report.
...
- State how the change was verified, including which backends the specs were run against. Never claim broader verification than what ran: "verified on both backends" with specs scoped `it_only_db('mongo')` is a contradiction reviewers will catch.
- Disclose AI authorship; the project's terms require it. Before opening a pull request, search existing open and merged pull requests for the same fix, because a duplicate of an already-merged change is closed as spam.
- A pull request must arrive merge-ready. Maintainers offer conceptual guidance and review the concept, not the code quality; quality is the author's job before review, and a pull request with code-level issues is rejected rather than corrected through review rounds. If a review flags many issues, the answer is more planning and design, not another cycle: CI and the AI review are safeguards, not development tools.
- Human review time is the scarce resource, so avoid review ping-pong. Inspect and test generated code yourself before requesting review, and batch fixes rather than pushing one at a time.
- After opening, let the automated CodeRabbit review finish and address it before requesting a human review. Treat it as a sanity check rather than an arbiter: verify its claims against the code, and expect maintainers to independently re-derive anything it approved.
- Respond to review comments from AI reviewers as you would to a human reviewer's, and let the reviewer resolve their own comments. Only treat a thread as settled once the reviewer has accepted the commit or agreed with the explanation of why the feedback was wrong.
- Keep the description current. When scope or approach changes during review, update the description and verification notes to match; an outdated description is itself a review finding.
